...
class PlatformBuilder(object):
	def __init__(self, platformName, arches, configs, generateSwig, skipPremake):
...
class MultiArchBuilder(PlatformBuilder):
	def __init__(self, platformName, arches, configs, generateSwig, skipPremake):
		PlatformBuilder.__init__(self, platformName, arches, configs, generateSwig, skipPremake)
...
class XCodeDeviceBuilder(XCodeBuilder):
	def __init__(self, platformName, arches, configs, generateSwig, skipPremake):
		XCodeBuilder.__init__(self, platformName, arches, configs, generateSwig, skipPremake)
		self.supportedSdks = []
...
class WwiseUnityBuilder(object):
	...
	def CreatePlatformBuilder(self, platformName, arches, configs, generateSwig, skipPremake):
		builder = BuildModules[platformName].CreatePlatformBuilder(platformName, arches, configs, generateSwig, skipPremake, self.isMultiSDK)
		if builder is None:
	...
	def Build(self):
		...
		for p in self.platforms:
			builder = self.CreatePlatformBuilder(p, self.arches, self.configs, self.isGeneratingSwigBinding, self.isSkippingPremake)
			if builder is None:
